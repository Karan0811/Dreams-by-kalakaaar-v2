...
export function SignupForm() {
  ...
  async function onSubmit(values: SignupInput) {
    ...
    if (!bridgeResponse.ok) {
      const errorData = await bridgeResponse.json().catch(() => ({ error: { message: "Unknown error" } }));
      setFormError(errorData.error?.message || "Your account was created, but we couldn't finish setting it up. Please contact support before continuing — signing in may not work yet.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-[var(--space-300)]">
      ...
      <FormField
        ...
        error={errors.password?.message}
        required
        description="At least 12 characters, with an uppercase letter, a lowercase letter, and a number."
      >
        <PasswordInput autoComplete="new-password" {...register("password")} />
      </FormField>
